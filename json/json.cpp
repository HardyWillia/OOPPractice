...
/// Returns true if at the end of the file. bool is_eof(const char* first, const char* last) {
  return first == last;
}
/// Returns true if *first is a punctuation character. bool is_punctuation(const char* first, const char* last) 
{
...
/// Returns true if *first is a space character. bool is_space(const char* first, const char* last) {
  if (is_eof(first, last))
    return false;
  return std::isspace(*first);
}
/// Returns true if first delimits two words. bool is_delimiter(const char* first, const char* last) {
  return is_eof(first, last)
      || is_space(first, last) is_punctuation(first, last);
}
...
/// Skip whitespace. void skip_space(const char*& first, const char* last) {
...
/// Parse a value from a range of characters. Value* parse_value(const char*& first, const char* last); /// 
Parse the true value. Bool* parse_true(const char*& first, const char* last) {
  match_literal(first, last, "true");
  return new Bool(true);
}
/// Parse the false value. Bool* parse_false(const char*& first, const char* last) {
  match_literal(first, last, "false");
  return new Bool(false);
}
/// Parse the null value. Null* parse_null(const char*& first, const char* last) {
  match_literal(first, last, "null");
  return new Null();
}
/// Parse a string value. String* parse_string(const char*& first, const char* last) {
...
/// Parse a numeric value. Number* parse_number(const char*& first, const char* last) {
  const char* start = first++;
...
/// Parse an array. Array* parse_array(const char*& first, const char* last) {
...
/// Parse a key. This simply parses a string, ensuring that surrounding /// spaces are skipped. String* 
parse_key(const char*& first, const char* last) {
  space_guard g(first, last);
  return parse_string(first, last);
}
/// Parse an object. Object* parse_object(const char*& first, const char* last) {
...
/// The value parsed depends on the current character. Value* parse_value(const char*& first, const char* last) 
{
  if (is_eof(first, last))
    return nullptr;
